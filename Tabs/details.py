...
from utils import (get_count,get_percent_increase,df_combined,
                   demographics,social_media,col,category_orders,colors )
...
def show():
    ...
    with increase_graphs:
        def graph_percent_increase(dg,metric,weighted,title):
            df = get_percent_increase(df_combined,col[dg],col[metric],weighted)
        ...
        with graph3:
            fig = graph_percent_increase(dg,'Overall Sentiment',weighted,
                'Metric 3: Overall Sentiment<br><sub>Increase in "Negative" sentiment')
            st.plotly_chart(fig)
    ...
    with facet_graph:
        ...
        with filter1: metric = st.selectbox("Metric",['Economy Rating','Economy Outlook','Overall Sentiment'])
        ...
        df = get_count(df_combined,['Year',col[metric],col[dg]],weighted)
